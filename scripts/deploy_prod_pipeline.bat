@echo off
REM Script deploy_prod_pipeline.cmd
REM Script steps
...
if errorlevel 1 (
    echo Errore nel checkout di deploy_prod.
    exit /b 1
)
...
echo >>>>>>>>>>>> merge main in deploy_prod >>>>>>>>>>>> 
...
echo Push effettuato. Avviata pipeline deploy di produzione.
pause
